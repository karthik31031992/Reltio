...
public class LoadJsonToTenant {
						...
						futures.add(executorService.submit(new Callable<Long>() {
							...
							public Long call() {
								...
								try {
									...
									processResponse(result, dataloaderInput, stringToSend, uriWriter, totalRecordsSent,
											currentCount, reltioFileWriter, processTrackerService);
	 ...
	 * @throws IOException
	 */
	private static void processResponse(String result, DataloaderInput dataloaderInput, String stringToSend,
			ReltioCSVFileWriter uriWriter, List<Object> totalRecordsSent, int currentCount,
			ReltioFileWriter reltioFileWriter, ProcessTrackerService processTrackerService)
		...
		if (dataloaderInput.getIsAlwaysCreateDCR()) {
			if (!result.contains("\"uri\":\"changeRequests")) {
				 ...
				 */
				dataloadResponses = GSON.fromJson(result, new TypeToken<List<ReltioDataloadResponse>>() {
				}.getType());
			...
			if (result.contains("\"uri\":\"changeRequests")) {
				...
				/*
				 * Throw Error because the user has INITIATE_CHANGE_REQUST permission only. So
				 * stopping process execution
				 */

				logger.fatal("Error!!!  System is exiting.................");
				logger.fatal(
						"The user does not permission to create/update entities, but has permission to InitiateChangeRequests. ");
				logger.fatal(
						"Please give the user Create/Update permission or use ALWAYS_CREATE_DCR=true option to create changerequests only.");
				System.exit(-1);
				...
				for (ReltioDataloadResponse reltioDataloadResponse : dataloadResponses) {
					if (reltioDataloadResponse.getSuccessful()) {
						...
						ReltioCrosswalkObject crosswalkObject = GSON.fromJson(GSON.toJson(failedRec),
						...
						if (crosswalkObject != null && crosswalkObject.getCrosswalks() != null
							...
							dataloadErrors.setCrosswalkType(crosswalkObject.getCrosswalks().get(0).getType());
							dataloadErrors.setCrosswalkValue(crosswalkObject.getCrosswalks().get(0).getValue() + "");
						}
						List<ReltioDataloadErrors> reltioDataloadErrors = dataloaderInput.getDataloadErrorsMap()
								.get(dataloadErrors.getErrorCode());
						if (reltioDataloadErrors == null) {
							reltioDataloadErrors = new ArrayList<>();
						}
						reltioDataloadErrors.add(dataloadErrors);
						dataloaderInput.getDataloadErrorsMap().put(dataloadErrors.getErrorCode(), reltioDataloadErrors);
					}
				}
		...
		if (failCcount > 0) {
			logger.info("Success entities=" + sucCount + "|Total Sent entities=" + currentCount
					+ "| Failed Entity Count=" + failCcount + "|" + "ERROR:" + result + "\nFailure Count: " + failCcount
					+ "|" + GSON.toJson(failedRecords));
			reltioFileWriter.writeToFile("Failure Count: " + failCcount + "|" + GSON.toJson(failedRecords));

			for (Entry<Integer, List<ReltioDataloadErrors>> errorMap : dataloaderInput.getDataloadErrorsMap()
					.entrySet()) {
				if (errorMap.getValue().size() > MAX_FAILURE_COUNT) {
					dataloaderInput.setLastUpdateTime(sdf.format(System.currentTimeMillis()));
					dataloaderInput.setStatus("Aborted");
