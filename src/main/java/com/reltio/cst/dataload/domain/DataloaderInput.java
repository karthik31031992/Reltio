...
public class DataloaderInput implements Serializable {
    ...
    private List<String> requiredProps_s3 = Arrays.asList(
            "FAILED_RECORD_FILE",
            "RECORDS_PER_POST",
            "ENVIRONMENT_URL",
            "TENANT_ID",
    ...
    public DataloaderInput(Properties properties) {
        ...
        if (missingProps != null && missingProps.size() > 0) {
            ...
            System.exit(-1);
        } else if ( properties.getProperty("INPUTFILELOCATION").equalsIgnoreCase("s3") && missingProps_s3 != null && missingProps_s3.size() > 0) {
            logger.error("Process Aborted due to insufficient input properties... Below are the list of missing properties");
            logger.error(missingProps_s3);
        ...
        awsKey = properties.getProperty("AWSKEY");
        awsSecretKey = properties.getProperty("AWSSECRETKEY");
        bucket = properties.getProperty("BUCKET");
        inputfilelocation = properties.getProperty("INPUTFILELOCATION");
